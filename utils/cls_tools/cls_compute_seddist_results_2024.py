import os
from .SELD_evaluation_metrics_2024 import SELDMetrics, SELDMetricsSegmentLevel
from .cls_feature_class_2024 import FeatureClass
...
class ComputeSELDResults(object):
    def __init__(self, 
        ...
        for split in os.listdir(self._desc_dir): 
            if os.path.isdir(os.path.join(self._desc_dir, split)):
                ...
                gt_dict = self._feat_cls.load_output_format_file(os.path.join(self._desc_dir, split), cm2m=True)
                ...
                if self.segment_level:
                    ...
                    self._ref_labels[split] = [self._feat_cls.organize_labels(gt_dict, nb_ref_frames), nb_ref_frames]
    ...
    def get_SELD_Results(self, pred_files_path, is_jackknife=False):
        ...
        pred_labels_dict = {}
        if self.segment_level:
            eval = SELDMetricsSegmentLevel(nb_classes=self._feat_cls.get_nb_classes(),
                                                       doa_threshold=self._doa_thresh, average=self._average)
        else:
            eval = SELDMetrics(nb_classes=self._feat_cls.get_nb_classes(),
